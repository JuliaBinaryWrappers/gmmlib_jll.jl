...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import gmmlib_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("gmmlib_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "gmmlib_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"gmmlib", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "gmmlib_jll", "src", "gmmlib_jll.jl"))
        @info("gmmlib_ll dev'ed out to /depot/dev/gmmlib_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("09858cae-167c-5acb-9302-fddc6874d481"))
platforms = [Pkg.Artifacts.unpack_platform(e, "gmmlib", artifacts_toml) for e in artifacts["gmmlib"]]
...
if best_platform === nothing
    @debug("Unable to load gmmlib; unsupported platform $(triplet(platform_key_abi()))")
else
